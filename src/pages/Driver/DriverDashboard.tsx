...
// ✅ FIXED: Removed 'loading' from dependency array (Line 134) to prevent infinite loop
...
import { DriverTrip, DriverTripStats, ConfirmTripPayload } from '../../types/DriverTrip.types';
...
export default function DriverDashboard() {
  ...
  const fetchDriverData = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === 'active') {
        setTrips(tripsData.filter(t => t.booking_status !== 'completed' && t.booking_status !== 'cancelled'));
  ...
  }, [currentDriver, activeTab]);  // ← Removed 'loading' and 'fetchDriverData'
  ...
  const activeTrips = trips.filter(t => t.booking_status !== 'completed' && t.booking_status !== 'cancelled');
