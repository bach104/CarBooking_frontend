 ...
 * GET /api/driverTrip/trips/:driverId
 * 
 * ✅ FIX: Sửa path từ /driverTrip/trips/${driverId} (vẫn đúng)
 *         và xử lý response.data.data (ApiResponse format)
 */
export const getDriverTrips = async (driverId: string): Promise<DriverTrip[]> => {
  try {
    const response = await apiClient.get<ApiResponse<DriverTrip[]>>(`/driverTrip/trips/${driverId}`);
 ...
 * GET /api/driverTrip/stats/:driverId
 * 
...
export const getDriverTripStats = async (driverId: string): Promise<DriverTripStats> => {
  try {
    const response = await apiClient.get<ApiResponse<DriverTripStats>>(`/driverTrip/stats/${driverId}`);
